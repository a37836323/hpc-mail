...
- **外发只走 Cloudflare `send_email` binding**。`outbound.ts` 单通道；站内互投直接落库。发件域名接入 Cloudflare Email Sending 且账户使用 Workers Paid 后，该 binding 可发送到任意外部地址；只开启 Email Routing 时仍仅限账户中已验证的 destination。（Resend 兜底通道已按需求彻底移除。）
