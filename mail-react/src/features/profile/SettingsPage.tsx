...
export function SettingsPage() {
  ...
  if (profile.isPending) return <main className="mx-auto max-w-4xl p-4 sm:p-8"><div className="h-56 animate-pulse rounded-2xl bg-slate-100" /></main>
  if (profile.isError) return <main className="mx-auto max-w-4xl p-4 sm:p-8"><div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-5 text-sm text-red-800">账户信息加载失败。<Button className="ml-3" size="sm" variant="secondary" onClick={() => void profile.refetch()}>重试</Button></div></main>

  return (
    <main className="mx-auto w-full max-w-4xl p-4 pb-24 sm:p-8">
      <header className="mb-7"><p className="text-sm font-medium text-blue-700">账户</p><h1 className="mt-1 text-2xl font-semibold tracking-tight text-slate-950">个人设置</h1><p className="mt-2 text-sm text-slate-600">管理平台身份与登录安全。邮箱地址在邮箱管理中统一维护。</p></header>
      <div className="grid gap-5">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6" aria-labelledby="identity-title">
          <div className="mb-5 flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-blue-50 text-blue-700"><UserRound className="size-4" /></span><div><h2 id="identity-title" className="font-semibold text-slate-950">平台身份</h2><p className="text-sm text-slate-500">用户名用于登录，不代表任何邮箱。</p></div></div>
          <dl className="mb-5 grid gap-1 rounded-xl bg-slate-50 p-4 sm:grid-cols-[140px_1fr]"><dt className="text-sm text-slate-500">用户名</dt><dd className="break-all text-sm font-medium text-slate-900">{profile.data?.username}</dd></dl>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6" aria-labelledby="security-title">
