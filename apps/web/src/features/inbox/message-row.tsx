...
export function MessageRow({ message, variant }: { message: MessageSummary; variant: 'inbox' | 'sent' }) {
  const unread = variant === 'inbox' && !message.isRead;
  const primary = variant === 'inbox' ? message.fromName || message.fromAddress : message.address;
  ...
  return (
    <Link
      to={`/mail/${message.id}`}
      className={cn(
        'flex items-start gap-3 border-b border-line px-4 py-3 transition-colors hover:bg-surface-hover',
        unread ? 'bg-surface' : 'bg-surface',
      )}
    >
