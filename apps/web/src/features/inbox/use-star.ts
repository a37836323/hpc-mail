...
export function useStarMutation() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, starred }) => {
      ...
      const prevDetail = queryClient.getQueryData<MessageDetail>(queryKeys.messages.detail(id));
    ...
    onError: (_err, { id }, ctx) => {
      if (ctx?.prevDetail !== undefined) {
        queryClient.setQueryData(queryKeys.messages.detail(id), ctx.prevDetail);
      }
